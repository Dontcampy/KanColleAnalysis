...
class Communicator:
    def __init__(self, host, port):
        self.passwd = 'KanColle Mac'
        print 'scan started'
        if(port == '-1'): #No port, we need to scan
            logger.info('scan started')
            for i in range(1000, 2000):
                logger.info('scan on port %d' % i)
                self.dst = Server(host, str(i))
                try:
                    self.dst.connect()
                    self.dst.conn.settimeout(0.01)
                    tmp = self.dst.recv(4096)
                    if(tmp == self.passwd): #check password
                        port = str(i)
                        break
                    else:
                        del self.dst
        ...
        else: #will not work by default
            self.dst = Server(host, port)
            self.dst.connect()
        if port == '-1': #not connected
            logger.info('Data processor not connected!')
            self.status = False
        else:
            self.dst.send(self.passwd) #comfirm
    ...
    def send(self, data):
        if self.status:
            ...
            logger.info('WARNING no data processor connected')
    def recv():
        if self.status:
           return self.dst.recv()
        else:
            logger.info('WARNING no data processor connected')
...
class Proxy(multiprocessing.Process):
    ...
    def __init__(self, client, communicator):
...
class TCP(object):
    ...
    def __init__(self, communicator, hostname='127.0.0.1', port=8899, backlog=100):
    ...
    def run(self):
        try:
            ...
            logger.info('Starting server on address %s' % self.hostname)
...
class HTTP(TCP):
    ...
    def handle(self, client):
        proc = Proxy(client, self.communicator)
        proc.daemon = True
        proc.start()
...
def main():
    ...
    communicator = Communicator('127.0.0.1', '-1')
    
    hostname = args.hostname
    port = int(args.port)
    
    try:
        proxy = HTTP(communicator, hostname, port)
        proxy.run()
    except KeyboardInterrupt:
        pass
